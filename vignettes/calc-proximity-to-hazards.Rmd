...
This message warns us that our fires_sf are not projected. The `get_proximity` function has done us the favor of projecting the `fires_sf` into the same CRS as the `ga` layer. Note, this message does not indicate an error. The function will still work when one of the input datasets is not projected because it assumes that unprojected dataset should have the same projections at the other input. However, if neither input has a projected CRS or if they have different CRS, `get_proximity` will return an error. 
