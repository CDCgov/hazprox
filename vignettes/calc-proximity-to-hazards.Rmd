...
- describes how near an area is to all features of interest, with higher values indicating that an area is overall nearer to features than areas with lower proximity values
...
In this example, we will calculate and analyze the cumulative proximity to fires occurring in 2019 for census tracts in the state of Georgia. A simple feature dataset representing all 2020 census tracts in the State of Georgia is LazyLoaded with `hazprox` and can be accessed directly as shown below. The Georgia census tracts, `ga` are provided as a sf MULTIPOLYGON object, allowing us to perform geospatial analysis of these data directly. Viewing the `ga` header reveals that the census tracts are projected in NAD 1983 Georgia Statewide Lambert.
...
fires_sf |>
  arrange(severity) |>
  ggplot() +
  geom_sf(data = ga, color = NA, fill = "gray") +
  geom_sf(aes(size = severity, color = severity), alpha = 0.45) +
  scale_color_distiller(palette = "Spectral", limits = c(0, 7), breaks = seq(0, 7, by = 2), guide = 'legend') +
