test_that("basic sync_projection works", {
  ...
  x <- sync_projection(ga_2960, ga_2960)
  y <- sync_projection(ga_2960, ga_gcrs)
  ...
  expect_error(sync_projection(ga_2960, ga_3857), "Inputs have different projections. Transform data before proceeding.")
...
test_that("Non-sf objects return error", {
  x <- matrix(1:4, 2, 2)
  expect_error(sync_projection(ga_2960, 2))
  expect_error(sync_projection(ga_2960, "A"))
  expect_error(sync_projection(ga_2960, x))
})
